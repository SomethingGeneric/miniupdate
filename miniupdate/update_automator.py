...
class UpdateAutomator:
    ...
    def __init__(self, config: Config):
        ...
        if self.proxmox_config:
            try:
                ...
                self.vm_mapper = VMMapper(
                    self._resolve_vm_mapping_path(vm_mapping_file)
                )
    ...
    def process_host_automated_update(
        ...
        try:
            ...
            if self.vm_mapper:
                ...
                if not vm_mapping:
                    logger.warning(
                        f"No VM mapping found for {host.name} - snapshots disabled"
                    )
            ...
            with SSHManager(self.ssh_config) as ssh_manager:
                ...
                if not connection:
                    return AutomatedUpdateReport(
                        ...
                        update_report=UpdateReport(
                            host, None, [], error="Failed to connect via SSH"
                        ),
                ...
                if not os_info:
                    return AutomatedUpdateReport(
                        ...
                        vm_mapping=vm_mapping,
                        update_report=UpdateReport(
                            host, None, [], error="Failed to detect OS"
                        ),
                        result=UpdateResult.FAILED_UPDATES,
                        snapshot_name=None,
                ...
                for attempt in range(1, max_retries + 1):
                    if package_manager.refresh_cache():
                        break
                    logger.warning(
                        f"Failed to refresh package cache on {host.name} (attempt {attempt}/{max_retries})"
                    )
                    ...
                    return AutomatedUpdateReport(
                        ...
                        vm_mapping=vm_mapping,
                        update_report=UpdateReport(
                            host, os_info, [], error=error_details
                        ),
                        result=UpdateResult.FAILED_UPDATES,
                        snapshot_name=None,
                ...
                if is_opt_out_host or not self.update_config.get(
                    "apply_updates", False
                ):
                    if is_opt_out_host:
                        logger.info(
                            f"Host {host.name} is in opt-out list - only checking updates"
                        )
                        result = UpdateResult.OPT_OUT
                    else:
                        logger.info(
                            f"Update application disabled - only checking updates on {host.name}"
                        )
                        result = (
                            UpdateResult.OPT_OUT
                        )  # Treat global disable same as opt-out
                    return AutomatedUpdateReport(
                        host=host,
                        vm_mapping=vm_mapping,
                ...
                logger.info(
                    f"Found {len(updates)} updates for {host.name} "
                ...
                if self.update_config.get("reboot_after_updates", False):
                    if snapshot_name:
                        logger.info(
                            f"Reboot after updates is enabled - proceeding with reboot for {host.name}"
                        )
                    ...
                    logger.info(
                        f"Reboot after updates is disabled - skipping reboot for {host.name}"
                    )
    ...
    def _create_snapshot(
        self, vm_mapping: VMMapping, start_time: datetime
    ) -> Optional[str]:
        ...
        try:
            ...
            if "data" in response and isinstance(response["data"], str):
                ...
                if self.proxmox_client.wait_for_task(
                    ...
                    logger.error(
                        f"Snapshot creation task failed for VM {vm_mapping.vmid}"
                    )
                    return None
            else:
                # Some versions might complete immediately
                logger.info(
                    f"Snapshot {snapshot_name} created for VM {vm_mapping.vmid}"
                )
                return snapshot_name
    ...
    def _revert_snapshot(self, vm_mapping: VMMapping, snapshot_name: str) -> bool:
        ...
        try:
            ...
            if not self.proxmox_client.start_vm(
                ...
                logger.error(
                    f"Failed to power on VM {vm_mapping.vmid} after snapshot restore"
                )
                return False

            logger.info(
                f"VM {vm_mapping.vmid} is now powered on after snapshot restore"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to revert VM {vm_mapping.vmid} to snapshot {snapshot_name}: {e}"
            )
            return False

    def _handle_reboot_and_verification(
        self,
        host: Host,
        vm_mapping: Optional[VMMapping],
        snapshot_name: Optional[str],
        start_time: datetime,
    ) -> Optional[AutomatedUpdateReport]:
        ...
        try:
            ...
            for snapshot in snapshots:
                ...
                try:
                    ...
                    automated_snapshots.append({"name": snap_name, "time": snap_time})
                except (ValueError, IndexError) as e:
                    logger.debug(
                        f"Could not parse snapshot timestamp for {snap_name}: {e}"
                    )
                    continue
            ...
            if vm_mapping.max_snapshots is not None:
                ...
                if len(automated_snapshots) > vm_mapping.max_snapshots:
                    snapshots_to_delete = automated_snapshots[
                        vm_mapping.max_snapshots :
                    ]
                    logger.info(
                        f"Per-host snapshot quota: keeping {vm_mapping.max_snapshots} newest snapshots "
                ...
                if len(automated_snapshots) > default_max_snapshots:
                    ...
                    logger.info(
                        f"Default snapshot quota: keeping {default_max_snapshots} newest snapshots "
                        f"for VM {vm_mapping.vmid}, deleting {len(snapshots_to_delete)} older ones"
                    )

                # Also apply time-based retention if configured (for additional cleanup)
                if not snapshots_to_delete:
                    retention_days = self.update_config.get(
                    ...
                    if snapshots_to_delete:
                        logger.info(
                            f"Time-based retention: deleting {len(snapshots_to_delete)} snapshots "
            ...
            logger.warning(
                f"Failed to cleanup old snapshots for VM {vm_mapping.vmid}: {e}"
            )
