...
class OSInfo:
    ...
    def __init__(self, os_family: str, distribution: str, version: str, 
                 package_manager: str, architecture: str = "unknown"):
    ...
    def __repr__(self):
        return (f"OSInfo(os_family='{self.os_family}', distribution='{self.distribution}', "
...
class OSDetector:
    ...
    def _get_os_release_info(self) -> Dict[str, str]:
        ...
        exit_code, stdout, stderr = self.connection.execute_command('cat /etc/os-release 2>/dev/null || true')
    ...
    def _get_lsb_info(self) -> Dict[str, str]:
        """Get LSB information."""
        exit_code, stdout, stderr = self.connection.execute_command('lsb_release -a 2>/dev/null || true')
        if exit_code != 0 or not stdout.strip():
            return {}
    ...
    def _parse_os_info(self, uname_info: Dict[str, str], 
                      os_release_info: Dict[str, str], 
                      lsb_info: Dict[str, str]) -> Tuple[str, str, str]:
