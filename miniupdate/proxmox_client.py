...
class ProxmoxClient:
    ...
    def _api_request(self, method: str, path: str, data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            if method.upper() == 'GET':
                response = self.session.get(url, params=data, verify=self.verify_ssl, timeout=self.timeout)
            elif method.upper() == 'POST':
                response = self.session.post(url, data=data, verify=self.verify_ssl, timeout=self.timeout)
            elif method.upper() == 'DELETE':
                response = self.session.delete(url, data=data, verify=self.verify_ssl, timeout=self.timeout)
            else:
            ...
            if response.status_code not in [200, 201]:
                raise ProxmoxAPIError(f"API request failed: {response.status_code} - {response.text}")
    ...
    def rollback_snapshot(self, node: str, vmid: int, snapname: str) -> Dict[str, Any]:
        ...
        logger.warning(f"Rolling back VM {vmid} on node {node} to snapshot '{snapname}'")
