...
class PackageUpdate:
    ...
    def __repr__(self):
        return (
            f"PackageUpdate(name='{self.name}', "
    ...
    def refresh_cache(self) -> bool:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logger.error(
                    f"Failed to refresh APT cache (exit code {exit_code}): {stderr.strip()}"
                )
                return False
...
class YumPackageManager(PackageManager):
    ...
    def refresh_cache(self) -> bool:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logger.error(
                    f"Failed to refresh YUM cache (exit code {exit_code}): {stderr.strip()}"
                )
                return False
        except Exception as e:
    ...
    def _parse_yum_output(self, output: str) -> List[PackageUpdate]:
        ...
        for line in output.strip().split("\n"):
            ...
            if (
                not line
                or line.startswith("Loaded plugins")
                or line.startswith("Loading mirror")
            ):
                continue
    ...
    def apply_updates(self) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # Apply updates non-interactively
            exit_code, stdout, stderr = self.connection.execute_command(
                "sudo yum update -y", timeout=1800  # 30 minutes for updates
            )
...
class DnfPackageManager(PackageManager):
    ...
    def refresh_cache(self) -> bool:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logger.error(
                    f"Failed to refresh DNF cache (exit code {exit_code}): {stderr.strip()}"
                )
                return False
        except Exception as e:
    ...
    def apply_updates(self) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # Apply updates non-interactively
            exit_code, stdout, stderr = self.connection.execute_command(
                "sudo dnf update -y", timeout=1800  # 30 minutes for updates
            )
    ...
    def refresh_cache(self) -> bool:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logger.error(
                    f"Failed to refresh Zypper cache (exit code {exit_code}): {stderr.strip()}"
                )
                return False
        except Exception as e:
...
class PackmanPackageManager(PackageManager):
    ...
    def refresh_cache(self) -> bool:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logger.error(
                    f"Failed to refresh Pacman cache (exit code {exit_code}): {stderr.strip()}"
                )
                return False
        except Exception as e:
    ...
    def apply_updates(self) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # Apply updates non-interactively
            exit_code, stdout, stderr = self.connection.execute_command(
                "sudo pacman -Su --noconfirm", timeout=1800  # 30 minutes for updates
            )
...
class PkgPackageManager(PackageManager):
    ...
    def refresh_cache(self) -> bool:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                logger.error(
                    f"Failed to refresh pkg cache (exit code {exit_code}): {stderr.strip()}"
                )
                return False
        except Exception as e:
    ...
    def apply_updates(self) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # Apply updates non-interactively
            exit_code, stdout, stderr = self.connection.execute_command(
                "sudo pkg upgrade -y", timeout=1800  # 30 minutes for updates
            )
...
class PkgAddPackageManager(PackageManager):
    ...
    def apply_updates(self) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            exit_code, stdout, stderr = self.connection.execute_command(
                "doas pkg_add -u", timeout=1800  # 30 minutes for updates
            )
...
def get_package_manager(
    connection: SSHConnection, os_info: OSInfo
